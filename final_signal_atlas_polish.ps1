...
$home = Read-Utf8NoBom $homePath

if ($home -notmatch "DocumentLocaleSync") {
  $home = $home -replace 'import \{ Header \} from "@/components/layout/Header";', 'import { Header } from "@/components/layout/Header";' + "`r`n" + 'import { DocumentLocaleSync } from "@/components/layout/DocumentLocaleSync";'
}

$home = $home -replace 'workCopy:\s*"Each card shows the project route, role, stack, proof signals, and next case-study action\. Generated visuals are temporary placeholders until real screenshots are captured\.",',
'workCopy: "Each card highlights the project route, role, stack, and proof signals that lead into the case study.",'

$home = $home -replace 'workCopy:\s*"Las tarjetas ahora usan visuales generados, alturas estables, m\\\\u00e9tricas, stack y llamadas a la acci\\\\u00f3n m\\\\u00e1s claras\. Despu\\\\u00e9s podemos reemplazar los visuales con capturas reales sin romper el layout\.",',
